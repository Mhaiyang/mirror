...
Aggregation Module.
  ...
  @Function: Performing mirror detection only context branch.
...
class PyramidROIAlign_classify(KE.Layer):
    ...
    def call(self, inputs):
        ...
        boxes_n6 = tf.concat([a, b], 2)
        level_boxes_n6 = tf.gather_nd(boxes_n6, ix)
        level_boxes_n6 = tf.stop_gradient(level_boxes_n6)
        n6_pooled = tf.image.crop_and_resize(
            feature_maps[4], level_boxes_n6, box_indices, self.pool_shape,
            method="bilinear")

        pooled = tf.concat([n2_pooled, n3_pooled, n4_pooled, n5_pooled, n6_pooled], axis=3)
...
class Two():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            model = KM.Model(inputs, outputs, name='Two')
