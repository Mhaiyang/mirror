...
Edge-based Module.
  ...
  @Function: Performing mirror detection using mask-rcnn with no mask branch.
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    feature_maps: List of feature maps from different layers of the pyramid,
                  [N2, N3, N4, N5]. Each has a different resolution.
    - image_meta: [batch, (meta data)] Image details. See compose_image_meta()
    pool_size: [28, 14, 14, 7, 7]
    ...
    x = PyramidROIAlign_classify(pool_size[4], name="pyramid_roi_classify")(
        [rois, image_meta] + feature_maps)

    # Two 1024 FC layers (implemented with Conv2D for consistency)
    x = KL.TimeDistributed(KL.Conv2D(1024, (7, 7), padding="valid"), name="mod_class_conv1")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mod_class_bn1')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(1024, (1, 1)), name="mod_class_conv2")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mod_class_bn2')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    ...
    mrcnn_class_logits = KL.TimeDistributed(KL.Dense(num_classes),
                                            name='mod_class_logits')(shared)

    mrcnn_probs = KL.TimeDistributed(KL.Activation("softmax"),
                                     name="mod_class")(mrcnn_class_logits)

    # BBox head
    # [batch, boxes, num_classes * (dy, dx, log(dh), log(dw))]
    x = KL.TimeDistributed(KL.Dense(num_classes * 4, activation='linear'),
                           name='mod_bbox_fc')(shared)
    # Reshape to [batch, boxes, num_classes, (dy, dx, log(dh), log(dw))]
    s = K.int_shape(x)
    mrcnn_bbox = KL.Reshape((s[1], num_classes, 4), name="mod_bbox")(x)
...
class Mod():
    """Encapsulates the mask-rcnn model with no mask branch functionality.
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            model = KM.Model(inputs, outputs, name='Mod')
